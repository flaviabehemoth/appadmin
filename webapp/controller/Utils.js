sap.ui.define([
	...
	var Utils = {
		...
		getAvailableProductsTable: function(oController) {
			return oController.getOwnerComponent().getRootControl().byId("Utenti").byId("table");
		},

		getSelectedProductsTable: function(oController) {
			return oController.getOwnerComponent().getRootControl().byId("UtentiSelezionati").byId("table");
		},
		...
		getSelectedItemContext: function(oTable, fnCallback) {
			...
			if (!oSelectedItem) {
				MessageToast.show("seleziona un utente");
				return;
			}
